...
module DhlEcommerceClient
  class TrackASinglePackageApi
    ...
    # @return [TrackusingpackageId]
    def trackusingpackage_id(package_id, opts = {})
      data, _status_code, _headers = trackusingpackage_id_with_http_info(package_id, opts)
      data
    end
    ...
    # @return [Array<(TrackusingpackageId, Fixnum, Hash)>] TrackusingpackageId data, response status code and response headers
    def trackusingpackage_id_with_http_info(package_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: TrackASinglePackageApi.trackusingpackage_id ...'
      end
      # verify the required parameter 'package_id' is set
      if @api_client.config.client_side_validation && package_id.nil?
        fail ArgumentError, "Missing the required parameter 'package_id' when calling TrackASinglePackageApi.trackusingpackage_id"
      end
      # resource path
      local_var_path = '/tracking/v4/package'
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path,
        ...
        :return_type => 'TrackusingpackageId')
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: TrackASinglePackageApi#trackusingpackage_id\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
